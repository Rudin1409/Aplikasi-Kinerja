...
<div class="mb-8">
    <nav class="flex items-center text-sm text-gray-500 mb-5">
        <a href="<?= base_url('admin/dashboard') ?>" class="flex items-center hover:text-amber-600 transition-colors">
        ...
        <a href="<?= $back_url ?>" class="flex items-center hover:text-amber-600 transition-colors">
            <ion-icon name="stats-chart-outline" class="mr-2 text-lg"></ion-icon>
            <span class="font-medium">Capaian IKU</span>
        </a>
        <ion-icon name="chevron-forward-outline" class="mx-3 text-gray-300 text-xs"></ion-icon>
        <div
            class="flex items-center text-amber-700 bg-amber-50 px-3 py-1 rounded-full border border-amber-100 shadow-sm">
            <ion-icon name="briefcase-outline" class="mr-2 mt-0.5"></ion-icon>
            <span class="font-bold">Master Mitra</span>
        </div>
    </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div
            ...
            <div class="absolute inset-0 overflow-hidden rounded-2xl">
                <div
                    class="absolute top-0 right-0 -mr-20 -mt-20 w-72 h-72 rounded-full bg-gradient-to-br from-amber-100 to-orange-50 opacity-60 blur-3xl group-hover:scale-110 transition-transform duration-700">
                </div>
            </div>
            <div class="relative z-10">
                <div class="mb-4">
                    ...
                    <?php if (isset($prodi_info) && $prodi_info): ?>
                        <p class="text-gray-600 flex items-center text-sm">
                            <ion-icon name="school-outline" class="mr-2 text-amber-500"></ion-icon>Data Mitra Prodi
                        </p>
                    <?php elseif (isset($jurusan_info) && $jurusan_info): ?>
                        <p class="text-gray-600 flex items-center text-sm">
                            <ion-icon name="business-outline" class="mr-2 text-amber-500"></ion-icon>Jurusan:
                            <?= esc($jurusan_info['nama_jurusan']) ?>
                        </p>
                    <?php else: ?>
                        <p class="text-gray-600 text-sm">Kelola data mitra dan kerjasama industri.</p>
                    <?php endif; ?>

                    <!-- IKU Usage Description -->
                    <div class="mt-3 p-3 bg-amber-50/70 rounded-xl border border-amber-100">
                        <div class="flex items-start text-xs text-amber-700">
                            ...
                            <div>
                                ...
                                <span class="text-amber-600">IKU 6 (Kerjasama Prodi)</span> &bull;
                                <span class="text-amber-600">IKU 1 (Tracer Work)</span> &bull;
                                <span class="text-amber-600">IKU 2 (Magang MBKM)</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-2 mt-auto">
                    <a href="<?= base_url('admin/mitra/import') . ($query_string ? '?' . $query_string : '') ?>"
                    ...
                    <div class="relative" id="export-dropdown-container">
                        ...
                        <div id="export-dropdown-menu"
                            ...
                            <a href="<?= base_url('admin/mitra/export') . ($query_string ? '?' . $query_string : '') ?>"
                                class="flex items-center px-4 py-2 text-gray-700 hover:bg-amber-50 hover:text-amber-600 transition text-sm">
                    ...
                    <a href="<?= base_url('admin/mitra/create') . ($query_string ? '?' . $query_string : '') ?>"
                        class="inline-flex items-center px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-lg shadow-md transition text-sm">
        ...
        <!-- Stats Card -->
        <div
            class="relative overflow-hidden text-white p-8 flex items-center justify-between shadow-lg transform hover:-translate-y-1 transition-transform duration-300 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 h-full">
            <div class="relative z-10">
                <p class="text-amber-100 text-sm font-semibold uppercase tracking-wider mb-2">Total Mitra</p>
                <h2 class="text-5xl font-black"><?= number_format($pager->getTotal('mitra') ?? 0) ?></h2>
                <p class="text-amber-100 text-sm mt-2">Mitra Kerjasama</p>
            </div>
            <div class="bg-white/20 p-5 rounded-full backdrop-blur-sm">
                <ion-icon name="briefcase" class="text-4xl"></ion-icon>
            </div>
        </div>
    </div>
</div>
...
<div id="bulk-action-bar"
    class="hidden mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl flex flex-wrap items-center justify-between gap-4 shadow-sm animate-pulse">
    <div class="flex items-center space-x-3">
        <div class="bg-amber-600 text-white px-3 py-1.5 rounded-lg font-bold text-sm">
...
<?php if (session()->getFlashdata('success') || session()->getFlashdata('error')): ?>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const bgClass = isError ? 'bg-red-500' : 'bg-amber-500';
...
<div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
    <div
        ...
        <form action="<?= base_url('admin/mitra') ?>" method="get" class="flex items-center space-x-2">
            ...
            <div class="relative">
                <select name="per_page" onchange="this.form.submit()"
                    class="pl-3 pr-8 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 appearance-none shadow-sm cursor-pointer font-medium text-gray-600">
            ...
            <div class="relative group">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <ion-icon name="search-outline"
                        class="text-gray-400 group-focus-within:text-amber-500 transition-colors"></ion-icon>
                </div>
                <input type="text" name="keyword" value="<?= esc($keyword) ?>" placeholder="Cari Nama Mitra..."
                    class="pl-10 pr-16 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 w-48 sm:w-64 transition-all shadow-sm">
                <button type="submit"
                    class="absolute inset-y-0 right-0 px-3 text-sm font-semibold text-amber-600 hover:text-amber-800 rounded-r-lg transition-colors">Cari</button>
            </div>
        </form>
    </div>

    <!-- Table with Checkboxes -->
    <form id="bulk-form" action="<?= base_url('admin/mitra/bulk-delete') ?>" method="post">
        <?= csrf_field() ?>
        <?php if ($nama_prodi): ?><input type="hidden" name="prodi" value="<?= esc($nama_prodi) ?>"><?php endif; ?>
        <?php if ($jenjang): ?><input type="hidden" name="jenjang" value="<?= esc($jenjang) ?>"><?php endif; ?>
        <?php if ($jurusan_kode): ?><input type="hidden" name="jurusan"
                ...
                <thead>
                    <tr class="bg-gray-50/50 text-xs text-gray-500 uppercase tracking-wider border-b border-gray-100">
                        ...
                        $createSortLink = function ($field, $label) use ($base_params, $sort_by, $sort_order) {
                            ...
                            $url = base_url('admin/mitra') . '?' . http_build_query($params);
                            ...
                            return "<a href='{$url}' class='group flex items-center justify-between hover:text-amber-600 transition-colors cursor-pointer'><span>{$label}</span><ion-icon name='{$icon}' class='ml-1 text-sm {$iconClass} group-hover:text-amber-500'></ion-icon></a>";
                        };
                        ?>

                        <th class="px-6 py-4 font-semibold"><?= $createSortLink('nama_mitra', 'Nama Mitra') ?></th>
                        <th class="px-6 py-4 font-semibold"><?= $createSortLink('bidang_usaha', 'Bidang Usaha') ?></th>
                        <th class="px-6 py-4 font-semibold"><?= $createSortLink('kota', 'Lokasi') ?></th>
                        <th class="px-6 py-4 font-semibold">Kontak</th>
                        <th class="px-6 py-4 font-semibold"><?= $createSortLink('jenis_kerjasama', 'Kerjasama') ?></th>
                        <th class="px-6 py-4 font-semibold text-center w-32">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 text-sm">
                    <?php $i = 1 + ($per_page * (($pager->getCurrentPage('mitra') ?? 1) - 1)); ?>
                    <?php foreach ($mitra as $m): ?>
                        <tr class="hover:bg-amber-50/30 transition-colors duration-200 group"
                            data-id="<?= esc($m['id']) ?>">
                            <td class="px-4 py-4 text-center">
                                <input type="checkbox" name="selected_ids[]" value="<?= esc($m['id']) ?>"
                                    class="row-checkbox h-4 w-4 rounded border-gray-300 text-amber-600 focus:ring-amber-500 cursor-pointer">
                            </td>
                            <td class="px-4 py-4 text-center text-gray-500 font-medium"><?= $i++ ?></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div
                                        class="h-10 w-10 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center font-bold text-xs mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                        <?= strtoupper(substr($m['nama_mitra'], 0, 2)) ?>
                                    </div>
                                    <div>
                                        <div class="font-bold text-gray-800"><?= esc($m['nama_mitra']) ?></div>
                                        <?php if (!empty($m['penanggung_jawab'])): ?>
                                            <div class="text-xs text-gray-500 mt-0.5">PJ: <?= esc($m['penanggung_jawab']) ?>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex w-fit items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-amber-50 text-amber-700 border border-amber-100">
                                    <?= esc($m['bidang_usaha']) ?>
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col space-y-0.5">
                                    <?php if (!empty($m['kota']) || !empty($m['provinsi'])): ?>
                                        <div class="flex items-center text-xs text-gray-600">
                                            <ion-icon name="location-outline" class="mr-1 text-gray-400"></ion-icon>
                                            <?= esc(implode(', ', array_filter([$m['kota'], $m['provinsi']]))) ?>
                                        </div>
                                    <?php endif; ?>
                                    <?php if (!empty($m['alamat'])): ?>
                                        <div class="text-xs text-gray-500 truncate max-w-[150px]"
                                            title="<?= esc($m['alamat']) ?>"><?= esc($m['alamat']) ?></div>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                    ...
                                    <?php if (!empty($m['email'])): ?>
                                        <div class="flex items-center text-xs text-gray-600">
                                            <ion-icon name="mail-outline"
                                                class="mr-1.5 text-gray-400"></ion-icon><?= esc($m['email']) ?>
                                        </div>
                                    <?php endif; ?>
                                    <?php if (!empty($m['no_telp'])): ?>
                                        <div class="flex items-center text-xs text-gray-600">
                                            <ion-icon name="call-outline"
                                                class="mr-1.5 text-gray-400"></ion-icon><?= esc($m['no_telp']) ?>
                                        </div>
                                    <?php endif; ?>
                                    <?php if (empty($m['email']) && empty($m['no_telp'])): ?>
                                        <span class="text-gray-400 text-xs italic">-</span>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <?php if (!empty($m['jenis_kerjasama'])): ?>
                                    <span
                                        class="inline-flex w-fit items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-blue-50 text-blue-700 border border-blue-100">
                                        <?= esc($m['jenis_kerjasama']) ?>
                                    </span>
                                <?php else: ?>
                                    <span class="text-gray-400 text-xs italic">-</span>
                                <?php endif; ?>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div
                                    ...
                                    <a href="<?= base_url('admin/mitra/edit/' . $m['id']) ?>"
                                        class="p-2 bg-white text-amber-600 rounded-lg border border-gray-200 hover:bg-amber-50 hover:border-amber-200 transition-all shadow-sm"
                                        title="Edit Data">
                                        <ion-icon name="create-outline" class="text-lg"></ion-icon>
                                    </a>
                                    <a href="<?= base_url('admin/mitra/delete/' . $m['id']) ?>"
    ...
    <form id="export-selected-form" action="<?= base_url('admin/mitra/export-selected') ?>" method="post"
        class="hidden">
        <?= csrf_field() ?>
    ...
    <div
        ...
        <span class="text-sm text-gray-600">
            Menampilkan <strong><?= count($mitra) ?></strong> dari <strong><?= $pager->getTotal('mitra') ?></strong>
            data
        </span>
        <?= $pager->links('mitra', 'default_full') ?>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateBulkUI() {
            ...
            if (count > 0) {
                ...
                btnExportSelected.classList.add('text-gray-700', 'hover:bg-amber-50', 'hover:text-amber-600');
            } else {
                bulkActionBar.classList.add('hidden');
                btnExportSelected.disabled = true;
                btnExportSelected.classList.add('text-gray-400', 'cursor-not-allowed');
                btnExportSelected.classList.remove('text-gray-700', 'hover:bg-amber-50', 'hover:text-amber-600');
            }
        }
    ...
    function deleteSelected() {
        ...
        if (confirm(`Apakah Anda yakin ingin menghapus ${checked.length} data mitra yang dipilih?`)) {
    ...
    function exportSelected() {
        ...
        document.getElementById('export-selected-ids').value = JSON.stringify(ids);
