...
<div class="mb-6">
    ...
    <div class="mb-2">
        <h1 class="text-3xl font-extrabold text-gray-800 tracking-tight leading-tight">Import Data Dosen</h1>
        <p class="text-gray-500 mt-2 text-sm">Upload data dosen menggunakan template Excel yang disediakan.</p>
    </div>
</div>

<!-- Upload Form -->
<form action="<?= base_url('admin/dosen/preview-import') ?>" method="post" enctype="multipart/form-data"
    ...
    <div class="p-8">
        ...
        <div class="bg-emerald-50 border-l-4 border-emerald-500 p-4 mb-6 rounded-r-lg">
            <p class="font-bold text-emerald-700 mb-2">Panduan Import:</p>
            <ul class="list-disc list-inside text-sm text-emerald-600 space-y-1">
                <li>Gunakan template Excel yang disediakan.</li>
                <li>Kolom Wajib: <strong>NIDN, Nama Lengkap, Kode Prodi</strong>.</li>
                <li>Kolom Opsional: Homebase, Gelar Depan, Gelar Belakang, Email, No HP.</li>
            </ul>

            <?php
            $template_url = base_url('admin/dosen/download-template');
        ...
        <div class="mb-6">
            ...
            <div class="flex items-center gap-4">
                <input type="file" name="file_excel" id="file_excel"
                    class="flex-1 border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition"
                    accept=".xlsx, .xls" required onchange="autoPreview()">
                <button type="submit" id="btn-preview"
                    class="px-6 py-3 bg-emerald-600 text-white font-bold rounded-lg hover:bg-emerald-700 transition shadow-md flex items-center">
        ...
        <!-- Loading Indicator -->
        <div id="loading-indicator" class="hidden flex items-center text-emerald-600 font-bold mt-2">
            <svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-emerald-600" xmlns="http://www.w3.org/2000/svg" fill="none"
                viewBox="0 0 24 24">
...
<?php if (!empty($preview_data)): ?>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-xl border border-gray-100">
            ...
            <table class="w-full text-left border-collapse relative">
                <thead class="sticky top-0 z-10">
                    <tr class="bg-gray-100 text-xs uppercase text-gray-600 border-b border-gray-200 shadow-sm">
                        <th class="px-4 py-3 font-semibold bg-gray-100">NIDN / Nama</th>
                        ...
                        <tr class="border-b border-gray-200 hover:bg-white transition <?= !$row['valid'] ? 'bg-red-50' : '' ?>">
                            <td class="px-4 py-3 align-top">
                                <div class="font-bold text-gray-800">
                                    <?= esc(($row['gelar_depan'] ? $row['gelar_depan'] . ' ' : '') . $row['nama'] . ($row['gelar_belakang'] ? ', ' . $row['gelar_belakang'] : '')) ?>
                                </div>
                                <div class="text-xs text-gray-500">
                                    <?= esc($row['nidn']) ?>
                                    <?php if ($row['status_dosen'] == 'Baru'): ?>
                                        <span
                            ...
                            <td class="px-4 py-3 align-top">
                                <?= esc($row['nama_prodi']) ?>
                                <span class="text-xs text-gray-500 block"><?= esc($row['homebase']) ?></span>
                            </td>
                            <td class="px-4 py-3 align-top text-xs text-gray-600">
                                ...
                                <?php if (!empty($row['no_hp'])): ?>
                                    <div class="flex items-center"><ion-icon name="call-outline" class="mr-1"></ion-icon>
                                        <?= esc($row['no_hp']) ?></div>
                                <?php endif; ?>
                                <?php if (empty($row['email']) && empty($row['no_hp'])): ?>
                                    <span class="text-gray-400">-</span>
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-3 align-top">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium <?= $row['status_dosen'] == 'Baru' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800' ?>">
                                    <?= esc($row['status_dosen']) ?>
                                </span>
                            </td>
        ...
        <!-- Save Form -->
        <form action="<?= base_url('admin/dosen/save-import') ?>" method="post">
            <?= csrf_field() ?>
            <?php if ($nama_prodi): ?><input type="hidden" name="prodi" value="<?= esc($nama_prodi) ?>"><?php endif; ?>
            <?php if ($jenjang): ?><input type="hidden" name="jenjang" value="<?= esc($jenjang) ?>"><?php endif; ?>
            <?php if ($jurusan_kode): ?><input type="hidden" name="jurusan"
